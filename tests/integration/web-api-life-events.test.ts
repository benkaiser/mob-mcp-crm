...
describe('Life events internal API (/web/api/life-events)', () => {
  let server: ReturnType<typeof createServer> | null = null;
  afterEach(() => { if (server) { server.stop(); server = null; } });

  it('requires authentication', async () => {
    server = createServer(persistent);
    const res = await raw(server.app, 'GET', '/web/api/life-events?contact_id=x');
    expect(res.status).toBe(401);
  });

  it('creates, gets, updates, lists, deletes and restores a life event', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const contactId = await makeContact(c);

    const create = await c.mutate('POST', '/web/api/life-events', {
      contact_id: contactId, event_type: 'career', title: 'New job', occurred_at: '2026-01-01',
    });
    expect(create.status).toBe(201);
    const id = JSON.parse(create.body).data.id;

    const get = await c.get(`/web/api/life-events/${id}`);
    expect(get.status).toBe(200);

    const upd = await c.mutate('PATCH', `/web/api/life-events/${id}`, { title: 'Promotion' });
    expect(upd.status).toBe(200);
    expect(JSON.parse(upd.body).data.title).toBe('Promotion');

    const list = await c.get(`/web/api/life-events?contact_id=${contactId}`);
    expect(list.status).toBe(200);
    expect(JSON.parse(list.body).meta.total).toBeGreaterThanOrEqual(1);

    const del = await c.mutate('DELETE', `/web/api/life-events/${id}`);
    expect(del.status).toBe(200);

    const gone = await c.get(`/web/api/life-events/${id}`);
    expect(gone.status).toBe(404);

    const restore = await c.mutate('POST', `/web/api/life-events/${id}/restore`);
    expect(restore.status).toBe(200);
  });

  it('returns 404 for a missing life event', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const res = await c.get('/web/api/life-events/nope');
  ...
  it('returns 422 on validation error (missing title)', async () => {
    ...
    const c = await makeClient(server);
    const contactId = await makeContact(c);
    const res = await c.mutate('POST', '/web/api/life-events', { contact_id: contactId, event_type: 'career' });
    expect(res.status).toBe(422);
    expect(JSON.parse(res.body).error.code).toBe('validation_error');
  });

  it('requires contact_id on list', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const res = await c.get('/web/api/life-events');
    expect(res.status).toBe(422);
  });
});
