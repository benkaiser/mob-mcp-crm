...
describe('Gifts internal API (/web/api/gifts)', () => {
  let server: ReturnType<typeof createServer> | null = null;
  afterEach(() => { if (server) { server.stop(); server = null; } });

  it('requires authentication', async () => {
    server = createServer(persistent);
    const res = await raw(server.app, 'GET', '/web/api/gifts');
    expect(res.status).toBe(401);
  });

  it('creates, gets, updates, lists, deletes and restores a gift', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const contactId = await makeContact(c);

    const create = await c.mutate('POST', '/web/api/gifts', {
      contact_id: contactId, name: 'Book', direction: 'giving', estimated_cost: 20,
    });
    expect(create.status).toBe(201);
    const id = JSON.parse(create.body).data.id;

    const get = await c.get(`/web/api/gifts/${id}`);
    expect(get.status).toBe(200);

    const upd = await c.mutate('PATCH', `/web/api/gifts/${id}`, { status: 'purchased' });
    expect(upd.status).toBe(200);
    expect(JSON.parse(upd.body).data.status).toBe('purchased');

    const list = await c.get(`/web/api/gifts?contact_id=${contactId}`);
    expect(list.status).toBe(200);
    expect(JSON.parse(list.body).meta.total).toBeGreaterThanOrEqual(1);

    const del = await c.mutate('DELETE', `/web/api/gifts/${id}`);
    expect(del.status).toBe(200);

    const gone = await c.get(`/web/api/gifts/${id}`);
    expect(gone.status).toBe(404);

    const restore = await c.mutate('POST', `/web/api/gifts/${id}/restore`);
    expect(restore.status).toBe(200);
  });

  it('returns 404 for a missing gift', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const res = await c.get('/web/api/gifts/nope');
    expect(res.status).toBe(404);
  });

  it('returns 422 on validation error (missing direction)', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const contactId = await makeContact(c);
    const res = await c.mutate('POST', '/web/api/gifts', { contact_id: contactId, name: 'Book' });
