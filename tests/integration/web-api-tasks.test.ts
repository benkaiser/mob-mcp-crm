...
describe('Tasks internal API (/web/api/tasks)', () => {
  let server: ReturnType<typeof createServer> | null = null;
  afterEach(() => { if (server) { server.stop(); server = null; } });

  it('requires authentication', async () => {
    server = createServer(persistent);
    const res = await raw(server.app, 'GET', '/web/api/tasks');
    expect(res.status).toBe(401);
  });

  it('creates, gets, updates, lists, deletes and restores a task', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const contactId = await makeContact(c);

    const create = await c.mutate('POST', '/web/api/tasks', { title: 'Buy gift', contact_id: contactId, priority: 'high' });
    expect(create.status).toBe(201);
    const id = JSON.parse(create.body).data.id;

    const get = await c.get(`/web/api/tasks/${id}`);
    expect(get.status).toBe(200);

    const upd = await c.mutate('PATCH', `/web/api/tasks/${id}`, { title: 'Buy a book' });
    expect(upd.status).toBe(200);
    expect(JSON.parse(upd.body).data.title).toBe('Buy a book');

    const list = await c.get(`/web/api/tasks?contact_id=${contactId}`);
    expect(list.status).toBe(200);
    expect(JSON.parse(list.body).meta.total).toBeGreaterThanOrEqual(1);

    const del = await c.mutate('DELETE', `/web/api/tasks/${id}`);
    expect(del.status).toBe(200);

    const gone = await c.get(`/web/api/tasks/${id}`);
    expect(gone.status).toBe(404);

    const restore = await c.mutate('POST', `/web/api/tasks/${id}/restore`);
  ...
  it('completes a task', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const create = await c.mutate('POST', '/web/api/tasks', { title: 'Do thing' });
    const id = JSON.parse(create.body).data.id;
    const complete = await c.mutate('POST', `/web/api/tasks/${id}/complete`);
  ...
  it('returns 404 for a missing task', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const res = await c.get('/web/api/tasks/nope');
  ...
  it('returns 422 on validation error (missing title)', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const res = await c.mutate('POST', '/web/api/tasks', { priority: 'high' });
