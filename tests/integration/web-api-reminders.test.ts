...
describe('Reminders internal API (/web/api/reminders)', () => {
  let server: ReturnType<typeof createServer> | null = null;
  afterEach(() => { if (server) { server.stop(); server = null; } });

  it('requires authentication', async () => {
    server = createServer(persistent);
    const res = await raw(server.app, 'GET', '/web/api/reminders');
    expect(res.status).toBe(401);
  });

  it('creates, gets, updates, lists, deletes and restores a reminder', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const contactId = await makeContact(c);

    const create = await c.mutate('POST', '/web/api/reminders', {
      contact_id: contactId, title: 'Call back', reminder_date: '2026-06-01',
    });
    expect(create.status).toBe(201);
    const id = JSON.parse(create.body).data.id;

    const get = await c.get(`/web/api/reminders/${id}`);
    expect(get.status).toBe(200);

    const upd = await c.mutate('PATCH', `/web/api/reminders/${id}`, { title: 'Call urgently' });
    expect(upd.status).toBe(200);
    expect(JSON.parse(upd.body).data.title).toBe('Call urgently');

    const list = await c.get('/web/api/reminders');
    expect(list.status).toBe(200);
    expect(JSON.parse(list.body).meta.total).toBeGreaterThanOrEqual(1);

    const del = await c.mutate('DELETE', `/web/api/reminders/${id}`);
    expect(del.status).toBe(200);

    const gone = await c.get(`/web/api/reminders/${id}`);
    expect(gone.status).toBe(404);

    const restore = await c.mutate('POST', `/web/api/reminders/${id}/restore`);
    expect(restore.status).toBe(200);
  });

  it('supports complete, snooze and dismiss lifecycle', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const contactId = await makeContact(c);

    const mk = async () => {
      const r = await c.mutate('POST', '/web/api/reminders', { contact_id: contactId, title: 'X', reminder_date: '2026-06-01' });
      return JSON.parse(r.body).data.id;
    };

    const a = await mk();
    const complete = await c.mutate('POST', `/web/api/reminders/${a}/complete`);
    expect(complete.status).toBe(200);
    expect(JSON.parse(complete.body).data.status).toBe('completed');

    const b = await mk();
    const snooze = await c.mutate('POST', `/web/api/reminders/${b}/snooze`, { new_date: '2026-07-01' });
    expect(snooze.status).toBe(200);
    expect(JSON.parse(snooze.body).data.status).toBe('snoozed');
    expect(JSON.parse(snooze.body).data.reminder_date).toBe('2026-07-01');

    const dSnoozeBad = await c.mutate('POST', `/web/api/reminders/${b}/snooze`, {});
    expect(dSnoozeBad.status).toBe(422);

    const d = await mk();
    const dismiss = await c.mutate('POST', `/web/api/reminders/${d}/dismiss`);
    expect(dismiss.status).toBe(200);
    expect(JSON.parse(dismiss.body).data.dismissed).toBe(true);
  });

  it('returns 404 for a missing reminder', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const res = await c.get('/web/api/reminders/nope');
    expect(res.status).toBe(404);
  });

  it('returns 422 on validation error (missing reminder_date)', async () => {
    server = createServer(persistent);
    const c = await makeClient(server);
    const contactId = await makeContact(c);
    const res = await c.mutate('POST', '/web/api/reminders', { contact_id: contactId, title: 'X' });
