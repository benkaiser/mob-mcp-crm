...
export function createServer(config: ServerConfig): {
  ...
  // Generated by `npm run build:docs`. Probe both development and bundled
  // locations the same way the Preact SPA does below.
  const docsDir = [
    path.join(__dirname, '../../dist-docs'), // dev: src/server → repo/dist-docs
    path.join(__dirname, '../dist-docs'), // bundle: dist → repo/dist-docs
    path.join(__dirname, 'docs'), // bundle: dist/docs
  ].find((p) => existsSync(p)) ?? path.join(__dirname, '../../dist-docs');
  app.get('/docs', (_req, res, next) => {
    res.sendFile(path.join(docsDir, 'index.html'), (err) => {
      if (err) next();
    });
  });
  app.use('/docs', express.static(docsDir, { extensions: ['html'] }));
