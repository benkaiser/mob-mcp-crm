...
export class ReminderService {
  ...
  create(userId: string, input: CreateReminderInput, isAutoGenerated = false): Reminder {
    ...
    recordAudit(this.db, userId, 'reminder', id, 'create', undefined, created);
    return created;
  }
  ...
  update(userId: string, id: string, input: UpdateReminderInput): Reminder | null {
    ...
    if (didChange && updated) recordAudit(this.db, userId, 'reminder', id, 'update', existing, updated);
    return updated;
  }
  ...
  complete(userId: string, id: string): Reminder | null {
    ...
    const updated = this.getById(userId, id);
    if (updated) recordAudit(this.db, userId, 'reminder', id, 'update', existing, updated);
    return updated;
  }
  ...
  dismiss(userId: string, id: string): boolean {
    ...
    if (changed) {
      const updated = this.getById(userId, id);
      recordAudit(this.db, userId, 'reminder', id, 'update', existing, updated ?? { ...existing, status: 'dismissed' });
    }
    return changed;
  }
  ...
  softDelete(userId: string, id: string): boolean {
    ...
    if (deleted) recordAudit(this.db, userId, 'reminder', id, 'delete', existing, undefined);
    return deleted;
  }
  ...
  removeAutoReminders(userId: string, contactId: string): number {
    ...
    const existing = existingRows.map((row) => this.mapRow(row));

    const result = this.db.prepare(`
      UPDATE reminders SET deleted_at = datetime('now'), updated_at = datetime('now')
      WHERE contact_id = ? AND is_auto_generated = 1 AND deleted_at IS NULL
    `).run(contactId);
    for (const reminder of existing.slice(0, result.changes)) {
      recordAudit(this.db, userId, 'reminder', reminder.id, 'delete', reminder, undefined);
    }
